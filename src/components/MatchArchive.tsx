...
export function MatchArchive({ matches, birthday }: { matches: Match[]; birthday: string }) {
  ...
  return <section className="archive-section" id="archive" aria-labelledby="archive-title">
    <div className="container section-space">
      ...
      </> : <div className="mosaic-empty"><span>Even the archive takes a breath.</span><p>{matches.length ? 'There are no matches for this combination. Try another result or season.' : 'Your birthday falls after our last recorded match. New memories are still being made.'}</p>{matches.length > 0 && <button className="text-button" onClick={() => { setResult('all'); setSeason('all'); setActive(0); }}>Show all my matches <ArrowUpRight size={15} /></button>}</div>}
